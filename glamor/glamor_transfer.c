...
static const glamor_facet glamor_facet_24_to_32 = {
    .name    = "24_to_32",
    .vs_vars =
        "in vec2 position;\n"
        "in vec2 texcoord;\n"
...
/*
 * Upload a (w x h) region of pixel data into the pixmap FBO at
 * (dst_x, dst_y) in FBO-local coordinates, with the fragment shader
 * replacing source alpha with 1.0.
 */
static void
glamor_upload_24_to_32(ScreenPtr screen,
                       ...
                       int bytes_per_pixel)
{
    glamor_program *prog = &glamor_priv->transfer_24_to_32_prog;
    ...
    if (!prog->prog) {
        if (!glamor_build_program(screen, prog,
                                  &glamor_facet_24_to_32,
                                  NULL, NULL, NULL)) {
            ErrorF("glamor: failed to build 24_to_32 program\n");
            return;
        }
    }

    /* Pass 1: upload raw bits (alpha untouched) into a staging texture. */
    ...
    /* Pass 2: blit through alpha-force shader into the pixmap FBO. */
                    ...
                    uint8_t *bits, uint32_t byte_stride)
{
    ...
    glamor_pixmap_loop(priv, box_index) {
        ...
        while (nbox--) {
            ...
            if (force_alpha) {
                glamor_upload_24_to_32(screen, glamor_priv, fbo, box,
                                       x1 - box->x1, y1 - box->y1,
                                       x2 - x1, y2 - y1,
                ...
                for (; y1 < y2; y1++, src_line += stride_px)
                    glTexSubImage2D(GL_TEXTURE_2D, 0,
                                    x1 - box->x1, y1 - box->y1,
                                    x2 - x1, 1,
                                    f->format, f->type,
