		   ...
		   void *data)
{
	...
	XLAssert(toplevel->state & StateIsMapped && toplevel->role);
			  ...
			  unsigned int height)
{
	...
	if (toplevel->state & StateNeedDecorationConfigure && toplevel->decoration)
...
HandleWmStateChange(XdgToplevel *toplevel)
{
	...
	for (i = 0; i < actual_size; ++i)
	{
		...
		if (states[i] == _NET_WM_STATE_MAXIMIZED_HORZ || states[i] == _NET_WM_STATE_MAXIMIZED_VERT)
...
HandleAllowedActionsChange(XdgToplevel *toplevel)
{
	...
	for (i = 0; i < actual_size; ++i)
	{
		...
		if (states[i] == _NET_WM_ACTION_MAXIMIZE_HORZ || states[i] == _NET_WM_ACTION_MAXIMIZE_VERT)
			toplevel->supported |= SupportsMaximize;
...
Attach(Role *role, XdgRoleImplementation *impl)
{
	...
	if (wl_resource_get_version(toplevel->resource) >= 5)
	{
	   ...
	   _NET_SUPPORTED.  */
		...
		if (XLWmSupportsHint(_NET_WM_STATE_MAXIMIZED_HORZ) || XLWmSupportsHint(_NET_WM_STATE_MAXIMIZED_VERT))
			toplevel->supported |= SupportsMaximize;
...
Commit(Role *role, Surface *surface, XdgRoleImplementation *impl)
{
	...
	if (!toplevel->conf_reply && toplevel->state & StateDecorationModeDirty)
	{
...
HandleConfigureEvent(XdgToplevel *toplevel, XEvent *event)
{
	...
	if (event->xconfigure.width == toplevel->width && event->xconfigure.height == toplevel->height)
	{
		if (!toplevel->configuration_timer)
...
UpdateParent(XdgToplevel *child, XdgToplevel *parent)
{
	...
	if (parent)
	{
		...
		child->parent_callback = RunOnUnmap(parent, HandleParentUnmapped, child);
	}
...
HandleResizeDone(void *key, void *data)
{
	...
	toplevel = data;
	toplevel->resize_callbacks = XLListRemove(toplevel->resize_callbacks, key);
		   ...
		   int32_t width, int32_t height)
{
	...
	if (toplevel->max_height != height || toplevel->max_width != width)
		toplevel->state |= StatePendingMaxSize;
}
		   ...
		   int32_t width, int32_t height)
{
	...
	if (toplevel->min_width != width || toplevel->min_height != height)
		toplevel->state |= StatePendingMinSize;
}
...
UnsetMaximized(struct wl_client *client, struct wl_resource *resource)
{
	...
	if (!toplevel->toplevel_state.maximized)
	{
	   ...
	   client.
		...
		if (!toplevel->configuration_timer && !(toplevel->state & StateWaitingForInitialConfigure))
			SendConfigure(toplevel, 0, 0);
...
Bool XLHandleXEventForXdgToplevels(XEvent *event)
{
	XdgToplevel *toplevel;
	XdgRoleImplementation *impl;
	...
	if (event->type == ClientMessage)
	{
		...
		{
			if (event->xclient.data.l[0] == WM_DELETE_WINDOW && toplevel->resource)
			{
	...
	if (event->type == ConfigureNotify)
	{
		...
		if (toplevel && toplevel->role && toplevel->role->surface && toplevel->state & StateIsMapped)
...
Bool XLIsXdgToplevel(Window window)
{
...
#undef AlreadyConstructed
	...
	if (toplevel->role->surface && toplevel->role->surface->current_state.buffer)
	{
