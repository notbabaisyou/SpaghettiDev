...
void XLDefaultCommit(Surface *surface)
{
...
CheckViewportValues(Surface *surface)
{
	...
	if (!surface->viewport || state->src_x == -1.0 || !state->buffer)
		return;
...
HandleScaleChanged(void *data, int new_scale)
{
	...
	if (subcompositor && surface->role && surface->role->funcs.subsurface_update)
...
SavePendingState(Surface *surface)
{
	...
	if (surface->pending_state.pending & PendingBufferScale)
		surface->cached_state.buffer_scale = surface->pending_state.buffer_scale;
	...
	{
		surface->cached_state.dest_width = surface->pending_state.dest_width;
		surface->cached_state.dest_height = surface->pending_state.dest_height;
	}
	...
	if (surface->pending_state.pending & PendingViewportSrc)
	{
		...
		surface->cached_state.src_width = surface->pending_state.src_width;
		surface->cached_state.src_height = surface->pending_state.src_height;
	}
...
InternalCommit1(Surface *surface, State *pending)
{
	...
	if (pending->pending & PendingPresentationHint)
		surface->current_state.presentation_hint = pending->presentation_hint;
	...
	if (pending->pending & PendingViewportSrc || pending->pending & PendingViewportDest)
	{
					 ...
					 uint32_t id)
{
	...
	surface->scale_callback_key = XLAddScaleChangeCallback(surface, HandleScaleChanged);
...
Bool XLSurfaceAttachRole(Surface *surface, Role *role)
{
	if (surface->role)
		return False;
...
void XLSurfaceReleaseRole(Surface *surface, Role *role)
{
...
void XLSurfaceRunFrameCallbacks(Surface *surface, struct timespec time)
{
	uint64_t ms_time;
	XLList *list;
...
void XLSurfaceRunFrameCallbacksMs(Surface *surface, uint32_t ms_time)
{
	XLList *list;
...
void XLSurfaceCancelCommitCallback(CommitCallback *callback)
{
...
void XLSurfaceCancelUnmapCallback(UnmapCallback *callback)
{
...
void XLSurfaceCancelRunOnFree(DestroyCallback *callback)
{
...
{
	if (!surface->role || !surface->role->funcs.get_window)
		return None;
...
{
	if (!surface->role || !surface->role->funcs.get_resize_dimensions)
		return False;
...
void XLSurfaceMoveBy(Surface *surface, int west, int north)
{
	if (!surface->role || !surface->role->funcs.move_by)
		return;
...
{
	if (!surface->role || !surface->role->funcs.select_extra_events)
		return;
...
void XLSurfaceNoteFocus(Surface *surface, FocusMode focus)
{
...
void XLSurfaceMergeCachedState(Surface *surface)
{
