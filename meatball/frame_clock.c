...
FreezeForValue(FrameClock *clock, uint64_t counter_value)
{
	...
	if (need_empty_frame
	   ...
	   commit after ack_configure.  */
		&& clock->can_forward_sync_counter && clock->can_forward_sync_counter(clock->freeze_callback_data))
...
PostEndFrame(FrameClock *clock)
{
	...
	clock_gettime(CLOCK_MONOTONIC_FAST, &current_time);
#else
...
NoteFakeFrame(Timer *timer, void *data, struct timespec time)
{
	...
	if (clock->in_frame && (clock->finished_frame_id == clock->next_frame_id))
	{
		clock->in_frame = False;
...
void XLFrameClockAfterFrame(FrameClock *clock,
...
Bool XLFrameClockStartFrame(FrameClock *clock, Bool urgent)
{
	return StartFrame(clock, urgent, True);
}

void XLFrameClockEndFrame(FrameClock *clock)
{
	EndFrame(clock);
}

Bool XLFrameClockFrameInProgress(FrameClock *clock)
{
	return clock->in_frame;
}

void XLFrameClockHandleFrameEvent(FrameClock *clock, XEvent *event)
{
	...
	if (event->xclient.message_type == _NET_WM_FRAME_DRAWN)
	{
	   ...
	   bits.  */
		...
		if (value == clock->finished_frame_id && clock->in_frame
	...
	if (event->xclient.message_type == _NET_WM_FRAME_TIMINGS)
	{
	   ...
	   frame_timings_id.  */
		...
		clock->last_presentation_time = (clock->frame_timings_drawn_time + event->xclient.data.l[2]);
	   ...
	   (u)int32_t.  */
		...
		if (clock->refresh_interval & (1U << 31) || clock->frame_delay == 0x80000000)
		{
	...
	if (event->xclient.message_type == WM_PROTOCOLS && event->xclient.data.l[0] == _NET_WM_SYNC_REQUEST && event->xclient.data.l[4] == 1)
	{
...
void XLFreeFrameClock(FrameClock *clock)
{
...
XLMakeFrameClockForWindow(Window window)
{
	...
	if (frame_sync_supported)
	{
		clock->primary_counter = XSyncCreateCounter(compositor.display,
													initial_value);
		clock->secondary_counter = XSyncCreateCounter(compositor.display,
													  initial_value);
	}
	else
	...
	if (getenv("MEATBALL_DEBUG_REFRESH_PREDICTION"))
		clock->predict_refresh = True;

	return clock;
}

Bool XLFrameClockSyncSupported(void)
{
	return frame_sync_supported;
}

Bool XLFrameClockCanBatch(FrameClock *clock)
{
...
void XLFrameClockSetPredictRefresh(FrameClock *clock)
{
...
void XLFrameClockDisablePredictRefresh(FrameClock *clock)
{
	/* This sets whether or not the frame clock should try to predict
	   when the compositing manager will draw a frame to display.
...
void XLFrameClockSetFreezeCallback(FrameClock *clock, void (*callback)(void *, Bool),
...
void XLFrameClockNoteConfigure(FrameClock *clock)
{
...
void XLStopCursorClockCallback(void *key)
{
	CursorClockCallback *callback;
...
void XLInitFrameClock(void)
{
	if (!getenv("MEATBALL_DISABLE_FRAME_SYNCHRONIZATION"))
