...
HandleIntegerValue(char *name, int value, uint32_t last_change_serial)
{
	...
	{
		if (!strcmp(listener->setting_name, name) && last_change_serial > listener->last_change_serial)
		{
...
ReadSettingsData(void)
{
	...
	if (rc != Success || actual_type != _XSETTINGS_SETTINGS || actual_format != 8 || !nitems_return)
	{
...
Bool XLHandleOneXEventForXSettings(XEvent *event)
{
	...
	else if (event->type == DestroyNotify && event->xdestroywindow.window == xsettings_window)
	{
...
void XLListenToIntegerSetting(const char *name, void (*callback)(int))
{
	IntegerValueListener *listener;
