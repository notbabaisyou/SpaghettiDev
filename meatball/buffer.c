...
void XLRetainBuffer(ExtBuffer *buffer)
{
	buffer->funcs.retain(buffer);
}

void XLDereferenceBuffer(ExtBuffer *buffer)
{
...
void XLReleaseBuffer(ExtBuffer *buffer)
{
...
void XLBufferCancelRunOnFree(ExtBuffer *buffer, void *key)
{
	buffer->destroy_listeners = XLListRemove(buffer->destroy_listeners, key);
	XLFree(key);
}

void XLPrintBuffer(ExtBuffer *buffer)
{
	if (buffer->funcs.print_buffer)
		buffer->funcs.print_buffer(buffer);
}

void ExtBufferDestroy(ExtBuffer *buffer)
{
	XLList *listener;
