...
TimestampFromClientTime(Time time)
{
	...
	if (time < current_time.milliseconds && (current_time.milliseconds - time) >= HalfMonth)
		timestamp.months += 1;
	else if (time > current_time.milliseconds && (time - current_time.milliseconds) >= HalfMonth)
		timestamp.months -= 1;
...
StartAlarms(XSyncCounter counter, XSyncValue current_value)
{
	...
	value_mask = (XSyncCACounter | XSyncCATestType | XSyncCAValue | XSyncCAEvents);
...
{
	if (notify->alarm != alarm_a || notify->alarm != alarm_b)
