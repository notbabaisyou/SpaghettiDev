		...
		int caret_x, int caret_y, int caret_width, int caret_height)
{
	...
	if (input->x + input->width >= view_width)
	{
		if (input->width > view_width / 2)
			...
			input->x -= (input->x + input->width - 1 - view_width);
...
DoGeometryAllocation(TextInput *input)
{
	...
	if (xim_style & XIMPreeditPosition)
	{
		...
		if (input->current_state.pending & PendingCursorRectangle)
		{
			...
			spot.y = (CurrentCursorY(input) + CurrentCursorHeight(input));
		}
		else
		{
			spot.x = 0;
	...
	else if (xim_style & XIMPreeditArea)
	{
		...
		if (!rc)
		{
			...
			if (input->current_state.pending & PendingCursorRectangle)
			{
				needed->x = (CurrentCursorX(input) + CurrentCursorWidth(input));
...
Commit(struct wl_client *client, struct wl_resource *resource)
{
	...
	else
	{
		...
		if (input->pending_state.pending & PendingCursorRectangle)
		{
			...
			input->current_state.cursor_width = input->pending_state.cursor_width;
			input->current_state.cursor_height = input->pending_state.cursor_height;
		...
		if (input->pending_state.pending & PendingSurroundingText)
		{
			...
			input->current_state.surrounding_text = input->pending_state.surrounding_text;
...
HandleResourceDestroy(struct wl_resource *resource)
{
	...
	if (input->client_info)
	{
	   ...
	   well.  */
		if (input->client_info->inputs.next == &input->client_info->inputs)
		{
...
GetClientInfo(struct wl_client *client, Seat *seat, Bool create)
{
	...
	info->seat_key = XLSeatRunOnDestroy(seat, HandleSeatDestroyed, info);
		   ...
		   XIMStyle status_style)
{
	...
	{
		if ((styles->supported_styles[i] & preedit_style) && (styles->supported_styles[i] & status_style))
...
ConvertKeyEvent(XIDeviceEvent *xev, XEvent *event)
{
	...
	event->xkey.state = ((xev->mods.effective & ~(1 << 13 | 1 << 14)) | (xev->group.effective << 13));
...
LookupString(TextInput *input, XEvent *event, KeySym *keysym_return)
{
	...
	{
		if ((status == XLookupKeySym || status == XLookupBoth) && keysym_return)
