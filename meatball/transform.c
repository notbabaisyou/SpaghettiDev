...
void MatrixTranslate(Matrix *transform, float tx, float ty)
{
	Matrix temp, copy;
...
void MatrixScale(Matrix *transform, float sx, float sy)
{
	Matrix temp, copy;
...
void MatrixRotate(Matrix *transform, float theta, float x, float y)
{
	Matrix temp, copy;
...
void MatrixMirrorHorizontal(Matrix *transform, float width)
{
	Matrix temp, copy;
...
void MatrixExport(Matrix *transform, XTransform *xtransform)
{
	/* M1 M2 M3     X
	   M4 M5 M6   * Y
	   M7 M8 M9     Z
...
#define Export(row, column) \
	xtransform->matrix[row][column] = XDoubleToFixed(Index(*transform, row, column))
