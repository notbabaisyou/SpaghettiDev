...
RunOneSurfaceAction(Subsurface *subsurface, SurfaceAction *subaction)
{
	...
	{
		if (subaction->other != subsurface->parent && !CheckSiblingRelationship(subsurface, subaction->other))
...
NoteSubsurfaceDesynchronous(Subsurface *subsurface, Bool apply_state)
{
	...
	if (subsurface->pending_commit && subsurface->role.surface && apply_state)
	{
...
MaybeUpdateOutputs(Subsurface *subsurface)
{
	...
	if (subsurface->role.surface->output_x == INT_MIN || subsurface->role.surface->output_y == INT_MIN)
...
SubsurfaceUpdate(Surface *surface, Role *role)
{
	...
	if (!subsurface->parent || !subsurface->parent->role || !subsurface->parent->role->funcs.subsurface_update)
		return;
...
GetWindow(Surface *surface, Role *role)
{
	...
	if (!subsurface->parent || !subsurface->parent->role || !subsurface->parent->role->funcs.get_window)
		return None;
...
Teardown(Surface *surface, Role *role)
{
	...
	if (subsurface->parent)
	{
		...
		subsurface->parent->subsurfaces = XLListRemove(subsurface->parent->subsurfaces, surface);
			  ...
			  struct wl_resource *parent_resource)
{
	...
	subsurface->commit_callback = XLSurfaceRunAtCommit(parent, AfterParentCommit, subsurface);
	subsurface->synchronous = True;
...
void XLSubsurfaceHandleParentCommit(Surface *parent)
{
	SurfaceActionClientData *client;
...
void XLUpdateOutputsForChildren(Surface *parent, int base_x, int base_y)
{
	XLList *item;
	Subsurface *subsurface;
	Surface *child;
...
void XLUpdateDesynchronousChildren(Surface *parent, int *n_children)
{
	XLList *item;
	Subsurface *subsurface;
	Surface *child;
