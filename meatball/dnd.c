...
SendStatus(void)
{
	...
	{
		if ((dnd_state.used_action != WL_DATA_DEVICE_MANAGER_DND_ACTION_NONE) && dnd_state.accepted)
...
UpdateUsedAction(void)
{
	...
	{
		intersection = (dnd_state.supported_actions & dnd_state.source_actions);
...
RespondToDndDrop(void)
{
	...
	if (dnd_state.proto >= 5 && dnd_state.used_action && dnd_state.accepted && dnd_state.seat && dnd_state.respond)
	{
			 ...
			 int *x_out, int *y_out)
{
	...
	if (child)
	{
		...
		dnd_state.unmap_callback = XLSurfaceRunAtUnmap(child, HandleChildUnmap, NULL);
...
void XLDndWriteAwarenessProperty(Window window)
{
...
Bool XLDndFilterClientMessage(Surface *surface, XEvent *event)
{
		 ...
		 xcb_shape_get_rectangles_reply_t *input)
{
	...
	mask = (entry->old_event_mask | SubstructureNotifyMask | PropertyChangeMask);
...
HandleConfigureNotify(WindowCache *cache, XEvent *event)
{
	...
	else if (window->next == parent->children || window->next->window != event->xconfigure.above)
	{
...
FindToplevelWindow1(WindowCacheEntry *entry, int x, int y)
{
	...
	{
		if (XLIsWindowIconSurface(child->window) || !(child->flags & IsMapped))
			goto next;
					   ...
					   Window *proxy_return)
{
	...
	/* Otherwise, the properties were read.  Determine if they are
	   valid.  */
	if (proto->format == 32 && proto->type == XCB_ATOM_ATOM && xcb_get_property_value_length(proto) == 4)
	{
	...
	if (proxy->format == 32 && proxy->type == XCB_ATOM_WINDOW && xcb_get_property_value_length(proxy) == 4)
	{
...
SendEnter(void)
{
	...
	if (drag_state.toplevel == None || drag_state.version < 3)
		return;
...
ConvertActionsLoosely(uint32_t actions)
{
	...
	if (drag_state.modifiers & ShiftMask && actions & WL_DATA_DEVICE_MANAGER_DND_ACTION_MOVE)
...
HandleXdndStatus(XEvent *event)
{
	...
	if (!(drag_state.flags & WaitingForStatus) && drag_state.flags & PendingDrop)
	{
		...
		if (!(drag_state.flags & WillAcceptDrop) || drag_state.action == None)
		{
...
HandleXdndFinished(XEvent *event)
{
	...
	if (finish_version < 5 || event->xclient.data.l[0] & 1)
	{
	   ...
	   the data source, followed by finished.  */
		...
		if (wl_resource_get_version(resource) >= 3 && finish_version >= 5)
		{
...
void XLHandleOneXEventForDnd(XEvent *event)
{
	if (drag_state.window_cache)
...
void XLDoDragMotion(Seat *seat, double root_x, double root_y)
{
	...
	if (root_x == drag_state.last_root_x && root_y == drag_state.last_root_y)
