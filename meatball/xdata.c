...
HandleSelectionNotify(XFixesSelectionNotifyEvent *event)
{
	...
	if (event->selection == CLIPBOARD && TimeIs(event->selection_timestamp, Later, last_clipboard_change))
		/* This time is used to keep track of whether or not things like
		   disowning the selection were successful.  */
		last_clipboard_change = TimestampFromClientTime(event->selection_timestamp);

	if (event->selection == XA_PRIMARY && TimeIs(event->selection_timestamp, Later, last_primary_time))
		last_primary_time = TimestampFromClientTime(event->selection_timestamp);

	if (event->owner != None && event->selection == CLIPBOARD)
	...
	else if (event->owner != None && event->selection == XA_PRIMARY)
...
Bool XLHandleOneXEventForXData(XEvent *event)
{
...
SelectSelectionInput(Atom selection)
{
	...
	if (selection == CLIPBOARD && XGetSelectionOwner(compositor.display, CLIPBOARD) != None)
		NoticeClipboardChanged(time);

	if (selection == XA_PRIMARY && XGetSelectionOwner(compositor.display, XA_PRIMARY) != None)
...
NoticeConversionTransferWritable(int fd, void *data, WriteFd *writefd)
{
	...
	if (info->buffer_size)
	{
		...
		if (nconv == (size_t)-1 && errno != EINVAL)
		{
			...
			if ((errno == E2BIG && outsize == BUFSIZ) || errno != E2BIG)
			{
...
Bool XLOwnDragSelection(Time time, DataSource *source)
{
	Atom *targets;
	int ntargets;
	Bool rc;
...
void XLNoteSourceDestroyed(DataSource *source)
{
...
void XLNotePrimaryDestroyed(PDataSource *source)
{
...
#define NoteLocalSelectionBody(callback, time_1, time_2, atom, field, foreign, n_foreign) \
	...
	if (!time.months && !time.milliseconds)                                               \
		...
		return False;                                                                     \
                                                                                          \
	if (TimestampIs(time, Earlier, time_1) || TimestampIs(time, Earlier, time_2))         \
		/* TIME is out of date.  */                                                       \
		return False;                                                                     \
                                                                                          \
...
Bool XLNoteLocalPrimary(Seat *seat, PDataSource *source)
{
