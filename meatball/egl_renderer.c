...
static PFNEGLSETDAMAGEREGIONKHRPROC ISetDamageRegionKHR;
...
/* Whether buffer age is supported.  */
static Bool have_egl_ext_buffer_age;

#define EGL_DAMAGE_CAP_NONE				0
#define EGL_DAMAGE_CAP_SWAP_DAMAGE		1
#define EGL_DAMAGE_CAP_PARTIAL_UPDATE	2
static unsigned int damage_mode = 0;
...
#define LoadProc(name, ext, extname) \
	...
	I##name = (void *)eglGetProcAddress("egl" #name ext)

#define LoadProcGl(name, ext, extname) \
	if (epoxy_has_gl_extension(extname))      \
	I##name = (void *)eglGetProcAddress("gl" #name ext)
...
EglInitFuncs(void) {
	...
	LoadProc(SetDamageRegionKHR, "KHR", "EGL_KHR_partial_update");
}
	...
	for (i = 0; i < num_x_formats; ++i) {
		if (formats[i].depth > format->depth && formats[i].bits_per_pixel == bpp) {
			...
			found = XRenderFindFormat(compositor.display,
			                          PictFormatType | PictFormatDepth | PictFormatRed | PictFormatGreen |
			                          PictFormatBlue | PictFormatRedMask | PictFormatBlueMask | PictFormatGreenMask |
			                          PictFormatAlphaMask,
			                          &target_format, 0);
...
EglCompileShaders(void) {
	...
	clear_rect_program_pos_attrib = glGetAttribLocation(clear_rect_program, "pos");
	...
	if (params->flags & TransformSet) {
	   ...
	   system, scale u and v correctly if dimensions changed.  */
		...
		if (RotatesDimensions(params->transform))
			MatrixScale(&buffer->matrix, ((float) buffer->width / (float) buffer->height),
			            ((float) buffer->height / (float) buffer->width));
	}
...
Composite(RenderBuffer buffer, RenderTarget target,
	...
	/* Disable blending based on if an alpha channel is
	   present.  */
	if (op == OperationOver && egl_buffer->flags & HasAlpha)
		glEnable(GL_BLEND);
	else
		glDisable(GL_BLEND);
...
FinishRender(RenderTarget target, pixman_region32_t *damage,
	...
	switch (damage_mode) {
		...
		case EGL_DAMAGE_CAP_PARTIAL_UPDATE: {
			...
			ISetDamageRegionKHR(egl_display, egl_target->surface, rects, nboxes);
			/* We need to call eglSwapBuffers() after setting the damage region. */
			Fallthrough;
		}
...
TargetAge(RenderTarget target) {
	...
	if (have_egl_ext_buffer_age && eglQuerySurface(egl_display, egl_target->surface,
...
BufferFromDmaBuf(DmaBufAttributes *attributes, Bool *error) {
	...
	if (!IQueryDmaBufModifiers && attributes->modifier != DRM_FORMAT_MOD_INVALID)
		goto error;
...
ValidateShmParams(uint32_t format, uint32_t width, uint32_t height,
	...
	/* If the total size + offset is larger than the pool size,
	   return.  */
	if (IntAddWrapv(offset, total, &after) || after > pool_size)
		return False;
	...
	if (IntMultiplyWrapv((size_t)width, info->bpp / 8, &min_stride) || stride < min_stride
