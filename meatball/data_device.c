...
UpdateDeviceActions(DataDevice *device, DataSource *source)
{
	...
	if (!(modifiers & ShiftMask) && device->preferred_action & source->actions)
	...
	else
	{
	   ...
	   copy if Shift is held down.  */
		...
		if (modifiers & ShiftMask && intersection & WL_DATA_DEVICE_MANAGER_DND_ACTION_MOVE)
...
void XLDataDeviceClearSeat(DataDevice *device)
{
	device->seat = NULL;
}

void XLDataDeviceHandleFocusChange(DataDevice *device)
{
	SendDataOffersForDevice(device);
}

void XLSetForeignSelection(Timestamp time, CreateOfferFuncs functions)
{
	uint32_t serial;
	...
	if (current_selection_data && current_selection_data != &foreign_selection_key)
		wl_data_source_send_cancelled(current_selection_data->resource);
...
void XLClearForeignSelection(Timestamp time)
{
	if (TimestampIs(time, Earlier, foreign_selection_time))
		return;
...
int XLDataSourceTargetCount(DataSource *source)
{
	return source->n_mime_types;
}

void XLDataSourceGetTargets(DataSource *source, Atom *targets)
{
	int i;
	XIDList *list;
...
Bool XLDataSourceHasAtomTarget(DataSource *source, Atom target)
{
	XIDList *list;
...
Bool XLDataSourceHasTarget(DataSource *source, const char *mime_type)
{
	XLList *list;
...
void XLDataDeviceSendLeave(Seat *seat, Surface *surface, DataSource *source)
{
	DataDevice *device;
	DataDeviceReference *reference;
...
void XLDataDeviceSendDrop(Seat *seat, Surface *surface)
{
	DataDevice *device;
	DataDeviceReference *reference;
...
void XLDataSourceAttachDragDevice(DataSource *source, DataDevice *device)
{
	if (source->drag_device)
	{
...
void XLDataSourceSendDropPerformed(DataSource *source)
{
	if (wl_resource_get_version(source->resource) >= 3)
		wl_data_source_send_dnd_drop_performed(source->resource);
}

void XLDataSourceSendDropCancelled(DataSource *source)
{
	if (wl_resource_get_version(source->resource) >= 3)
		wl_data_source_send_cancelled(source->resource);
}

Bool XLDataSourceCanDrop(DataSource *source)
{
	...
	if (source->state & Version3Supported)
		return (source->state & ActionsSent && source->state & TypeAccepted);
...
void XLDataSourceUpdateDeviceActions(DataSource *drag_source)
{
