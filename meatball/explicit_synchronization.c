...
void XLSyncRelease(SyncRelease *release)
{
	Bool error;
	int fd;
...
void XLSyncCommit(Synchronization *synchronization)
{
	HandleSurfaceCommit(synchronization, synchronization->surface);
}

void XLWaitFence(Surface *surface)
{
	RenderFence fence;
	Bool error;
...
void XLInitExplicitSynchronization(void)
{
