...
NoteBounds(void *data, int min_x, int min_y, int max_x, int max_y)
{
	...
	if (min_x != icon->min_x || min_y != icon->min_y || max_x != icon->max_x || max_y != icon->max_y)
	{
...
Bool XLHandleOneXEventForIconSurfaces(XEvent *event)
{
	IconSurface *icon;
...
void XLMoveIconSurface(IconSurface *surface, int root_x, int root_y)
{
	MoveWindowTo(surface, root_x, root_y);
}

void XLInitIconSurfaces(void)
{
...
void XLReleaseIconSurface(IconSurface *icon)
{
...
Bool XLIsWindowIconSurface(Window window)
{
