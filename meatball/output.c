...
SendUpdates(Output *output, int difference)
{
	...
	for (tem = output->resources; tem; tem = tem->next)
	{
		...
		if (difference & NameChanged && wl_resource_get_version(tem->data) >= 3)
...
NoticeOutputsMaybeChanged(void)
{
	...
	if (any_change)
	{
		...
		for (; surface != &all_surfaces; surface = surface->next)
		{
			...
			if (surface->role && surface->role->funcs.outputs_changed)
...
Bool XLHandleOneXEventForOutputs(XEvent *event)
{
...
void XLOutputGetMinRefresh(struct timespec *timespec)
{
	Output *output;
	XLList *list;
...
void XLRemoveScaleChangeCallback(void *key)
{
	ScaleChangeCallback *callback;
...
void XLClearOutputs(Surface *surface)
{
	Output *output;
	int i;
	struct wl_resource *resource;
...
void XLOutputSetChangeFunction(void (*change_func)(Time))
{
...
void XLOutputHandleScaleChange(int new_scale)
{
