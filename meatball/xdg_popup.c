...
Detach(Role *role, XdgRoleImplementation *impl)
{
	...
	if (popup->parent)
	{
		if (popup->state & StateIsGrabbed || popup->state & StatePendingGrab)
...
AckConfigure(Role *role, XdgRoleImplementation *impl, uint32_t serial)
{
	...
	if (serial == popup->position_serial && popup->state & StateAckPosition)
	{
...
CheckCanGrab(Role *parent, Seat *seat)
{
	...
	if (type == TypePopup)
	{
		...
		return (popup->state & StateIsGrabbed && popup->grab_holder == seat);
	}
...
Dismiss(XdgPopup *popup, Bool do_parents)
{
	...
	if (popup->state & StateIsGrabbed && popup->parent)
		RevertGrabTo(popup, popup->parent);
...
Bool XLHandleXEventForXdgPopups(XEvent *event)
{
