...
Bool XLPDataSourceHasAtomTarget(PDataSource *source, Atom target)
{
	XIDList *list;
...
Bool XLPDataSourceHasTarget(PDataSource *source, const char *mime_type)
{
	XLList *list;
...
int XLPDataSourceTargetCount(PDataSource *source)
{
	return source->n_mime_types;
}

void XLPDataSourceGetTargets(PDataSource *source, Atom *targets)
{
	int i;
	XIDList *list;
...
void XLPrimarySelectionHandleFocusChange(Seat *seat)
{
	PDataDevice *device;
	struct wl_client *client;
	...
	while (device != &all_devices)
	{
		...
		if (device->flags & WasSentOffer && !XLSeatIsClientFocused(device->seat, client))
		{
		  ...
		  uint32_t id, struct wl_resource *seat)
{
	...
	/* If the primary selection is set and the client is focused, send
	   the data offer now.  */
	if (primary_selection && device->seat && XLSeatIsClientFocused(device->seat, client))
		UpdateForSingleReference(device);
}
...
void XLSetForeignPrimary(Timestamp time, CreateOfferFuncs functions)
{
	uint32_t serial;
	struct wl_resource *scratch;
	...
	if (primary_selection && primary_selection != &foreign_selection_key)
	{
		scratch = primary_selection->resource;
		zwp_primary_selection_source_v1_send_cancelled(scratch);
	}
...
void XLClearForeignPrimary(Timestamp time)
{
	struct wl_resource *scratch;
