...
GetWantedSynchronizationType(SyncHelper *helper)
{
	...
	/* If tearing is allowed, then use present targeting the next
	   frame.  */
	if (helper->role->surface && (helper->role->surface->current_state.presentation_hint == PresentationHintAsync))
...
void SyncHelperUpdate(SyncHelper *helper)
{
...
void FreeSyncHelper(SyncHelper *helper)
{
...
void SyncHelperHandleFrameEvent(SyncHelper *helper, XEvent *event)
{
...
void SyncHelperNoteConfigureEvent(SyncHelper *helper)
{
...
void SyncHelperCheckFrameCallback(SyncHelper *helper)
{
	uint64_t time;
...
void SyncHelperClearPendingFrame(SyncHelper *helper)
{
