...
MakeSubcompositor(void)
{
	...
	subcompositor->destroy_callbacks.next = &subcompositor->destroy_callbacks;
	subcompositor->destroy_callbacks.last = &subcompositor->destroy_callbacks;
...
{
	if (view->subcompositor && !IsPartiallyMapped(view->subcompositor))
		return True;
...
void ViewInsert(View *view, View *child)
{
	...
	/* We don't know whether or not the subcompositor is partially
	   ...
	   it will be cleared upon the next update if the subcompositor is
	   not partially mapped.  */
	if (view->subcompositor && (IsViewUnmapped(child) || child->link != child->inferior))
	...
	/* Now, if the subcompositor is still not garbaged, damage each
	   inferior of the view.  */
	if (child->subcompositor && !IsGarbaged(child->subcompositor))
		DamageIncludingInferiors(child);
}

void ViewInsertAfter(View *view, View *child, View *sibling)
{
	View *parent;
	List *prior;
...
void ViewInsertBefore(View *view, View *child, View *sibling)
{
	/* Make child's parent view.  */
	child->parent = view;
...
void ViewInsertStart(View *view, View *child)
{
...
void ViewUnparent(View *child)
{
	...
	attached = (ViewVisibilityState(child, &mapped) && mapped);
...
void ViewSetSubcompositor(View *view, Subcompositor *subcompositor)
{
	List *list;
...
ViewAfterSizeUpdate(View *view)
{
	...
	if (!view->subcompositor || !ViewVisibilityState(view, &mapped) || !mapped)
		return;
...
void ViewMove(View *view, int x, int y)
{
	pixman_region32_t damage;
	int doflags;
	Bool mapped;
...
void ViewMoveFractional(View *view, double x, double y)
{
...
void ViewMap(View *view)
{
	...
	if (view->subcompositor && (view->link != view->inferior || view->buffer))
	{
...
void ViewDamageBuffer(View *view, pixman_region32_t *damage)
{
	...
	else
	{
		...
		if (IsViewported(view))
		{
			...
			if (crop_width == -1)
			{
				crop_width = (BufferWidthAfterTransform(view) * GetContentScale(view->scale));
				crop_height = (BufferHeightAfterTransform(view) * GetContentScale(view->scale));
			}
...
void ViewSetOpaque(View *view, pixman_region32_t *opaque)
{
...
void ViewSetScale(View *view, int scale)
{
	if (view->scale == scale)
		return;
...
void ViewSetTransform(View *view, BufferTransform transform)
{
	...
	if (RotatesDimensions(transform) != RotatesDimensions(old_transform))
...
ViewComputeTransform(View *view, DrawParams *params, Bool draw)
{
	...
	if (IsViewported(view))
	{
	   ...
	   buffer width/height.  */
		...
		{
			params->crop_width = (BufferWidthAfterTransform(view) * GetContentScale(view->scale));
			params->crop_height = (BufferHeightAfterTransform(view) * GetContentScale(view->scale));
		}
	}

	if ((view->fract_x != 0.0 || view->fract_y != 0.0) && draw)
	{
		params->flags |= OffsetSet;
...
Bool SubcompositorIsEmpty(Subcompositor *subcompositor)
{
	return (subcompositor->min_x == subcompositor->max_x && subcompositor->min_y == subcompositor->max_y);
}
...
SubcompositorUpdateAncillary(Subcompositor *subcompositor)
{
	...
	{
		update_opaque = subcompositor->opaque_change != NULL;
		update_input = subcompositor->input_change != NULL;
	...
	{
		update_opaque = (IsOpaqueDirty(subcompositor) && subcompositor->opaque_change);
		update_input = (IsInputDirty(subcompositor) && subcompositor->input_change);
						...
						Bool bail_on_draw)
{
	...
	if (view && view->cull_region)
	{
	   ...
	   views, present it.  */
		if (op != OperationSource || !TryPresent(view, view->cull_region, &transform))
		{
			if (bail_on_draw)
...
SubcompositorComposite(Subcompositor *subcompositor)
{
	...
	if ((age > 0 && !subcompositor->last_damage) || (age > 1 && !subcompositor->before_damage))
	{
...
void SubcompositorUpdate(Subcompositor *subcompositor)
{
...
void SubcompositorExpose(Subcompositor *subcompositor, XEvent *event)
{
	pixman_region32_t damage;
...
void SubcompositorGarbage(Subcompositor *subcompositor)
{
	SetGarbaged(subcompositor);
}
...
void SubcompositorFree(Subcompositor *subcompositor)
{
	SubcompositorDestroyCallback *next, *last;

	/* It isn't valid to call this function with children attached.  */
	XLAssert(subcompositor->children->next == subcompositor->children);
	XLAssert(subcompositor->inferiors->next == subcompositor->inferiors);
						...
						int *view_x, int *view_y)
{
		 ...
		 list = list->last)
	{
	   ...
	   input region with the bounds of the view.  */
		if (temp_x < 0 || temp_y < 0 || temp_x >= list->view->width || temp_y >= list->view->height)
			continue;
...
void ViewSetData(View *view, void *data)
{
	view->data = data;
}

void ViewSetMaybeResizedFunction(View *view, void (*func)(View *))
{
	view->maybe_resized = func;
}

void ViewTranslate(View *view, int x, int y, int *x_out, int *y_out)
{
	if (view->subcompositor)
	{
...
void SubcompositorInit(void)
{
	/* Nothing to do here... */
}

int SubcompositorWidth(Subcompositor *subcompositor)
{
	return subcompositor->max_x - subcompositor->min_x + 1;
}

int SubcompositorHeight(Subcompositor *subcompositor)
{
...
void SubcompositorRemoveDestroyCallback(SubcompositorDestroyCallback *callback)
{
...
void SubcompositorSetAlwaysGarbaged(Subcompositor *subcompositor)
{
	SetGarbaged(subcompositor);
