...
MaybeCloseTopEdge(BarrierLine *lines, int *idx, int max_lines)
{
  ...
  lines[*idx].x1  |                                                | lines[*idx].x2
	   ...
	   case C.  */
		...
		else if (tem->x1 >= lines[*idx].x1 && tem->x2 <= lines[*idx].x2)
		{
	...
	for (i = 0; i < nrects; ++i)
	{
	   ...
	   towards another rectangle.  */
		...
		if (!l)
		{
			...
			lines[l].edges = (TopEdgeClosed | LeftEdgeClosed | RightEdgeClosed | BottomEdgeClosed);
			...
			if (lines[l - 1].y1 == boxes[i].y1)
			{
			   ...
			   == last.x2, extend last to x2.  */
				...
				else
				{
					...
					lines[l].edges = (LeftEdgeClosed | BottomEdgeClosed | RightEdgeClosed);
				}
		  ...
		  struct wl_resource *region_resource)
{
	...
	else
	{
		if (!confinement->pending_region)
		{
			confinement->pending_region = XLMalloc(sizeof *confinement->pending_region);
...
#undef AlreadyConstrained
	...
	confinement->seat_key = XLSeatRunOnDestroy(seat, HandleSeatDestroyed, confinement);
...
void XLPointerBarrierLeft(Seat *seat, Surface *surface)
{
	PointerConfinementDataRecord *record;
	PointerConfinement *confinement;
...
void XLPointerConstraintsSubsurfaceMoved(Surface *surface)
{
	int root_x, root_y;
...
void XLPointerConstraintsReconfineSurface(Surface *surface)
{
	int root_x, root_y;
