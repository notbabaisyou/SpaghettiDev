...
FindProvider(RRProvider id)
{
	...
	while (device != &all_devices)
	{
		if (!(device->flags & IsRemoved) && device->provider == id)
			return device;
	}
	...
	else
	{
	   ...
	   still present.  */
		...
		for (i = 0; i < tree->nconnectors[index]; ++i)
		{
			...
			if (connector->flags & IsDisconnected && info[i]->connection != XCB_RANDR_CONNECTION_DISCONNECTED)
			{
			...
			else if (!(connector->flags & IsDisconnected) && info[i]->connection == XCB_RANDR_CONNECTION_DISCONNECTED)
			{
...
void XLInitDrmLease(void)
{
	...
	if (reply->major_version < 1 || (reply->major_version == 1 && reply->minor_version < 6))
	{
		free(reply);
		return;
	}
