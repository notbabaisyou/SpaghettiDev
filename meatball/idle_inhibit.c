...
DetectSurfaceIdleInhibit(void)
{
	...
	while (inhibitor != &all_inhibitors)
	{
		...
		else if (inhibitor->surface->role && inhibitor->surface->role_type == SubsurfaceType)
		{
			...
			if (root && ViewIsVisible(inhibitor->surface->view) && root->num_focused_seats)
				ChangeInhibitionTo(IdleInhibited);
		}
...
void XLIdleInhibitNoticeSurfaceFocused(Surface *surface)
{
	NoticeSurfaceFocused(surface);
}

void XLDetectSurfaceIdleInhibit(void)
{
	DetectSurfaceIdleInhibit();
}
