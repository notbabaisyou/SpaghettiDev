...
#undef AlreadyInhibited
	...
	/* And the seat.  */
	inhibitor->seat = seat;
	inhibitor->seat_key = XLSeatRunOnDestroy(seat, HandleSeatDestroy, inhibitor);
	...
	/* If the given surface is the seat's focus, try to apply the grab
	   now.  */
	if (surface == XLSeatGetFocus(seat) && XLSeatApplyExternalGrab(seat, surface))
	{
...
void XLReleaseShortcutInhibition(Seat *seat, Surface *surface)
{
	KeyboardShortcutInhibitor *inhibitor;
