...
FreeBackBuffer(PictureTarget *target, BackBuffer *buffer) {
	...
	if (target->client && IntSubtractWrapv(target->client->n_pixels,
	                                       buffer->n_pixels,
...
CreateBackBuffer(PictureTarget *target) {
	...
	buffer->pixmap = XCreatePixmap(compositor.display, root_window,
	                               target->width, target->height,
	                               compositor.n_planes);
	buffer->picture = XRenderCreatePicture(compositor.display, buffer->pixmap,
	...
	if (target->client && IntAddWrapv(target->client->n_pixels, buffer->n_pixels,
	                                  &target->client->n_pixels))
...
AddAdditionalModifier(const char *name) {
	...
	for (i = 0; i < ArrayElements(known_modifiers) - 1; ++i) {
		if (!strcmp(known_modifiers[i].name, name)) {
			...
			user_specified_modifiers = XLRealloc(user_specified_modifiers,
			                                     num_specified_modifiers * sizeof *user_specified_modifiers);

			/* And add the modifier.  */
			user_specified_modifiers[num_specified_modifiers - 1] = known_modifiers[i].modifier;
			return;
		}
	}
...
TargetFromDrawable(Drawable drawable, Window window,
	...
	if (window != None)
		/* Start selecting for presentation events from the given
		   window.  */
		target->presentation_event_context = XPresentSelectInput(compositor.display, window,
		                                                         PresentIdleNotifyMask | PresentCompleteNotifyMask);
	else
...
NoteTargetSize(RenderTarget target, int width, int height) {
	...
	if (width != pict_target->width || height != pict_target->height) {
		...
		if (pict_target->client && IntMultiplyWrapv(width, height, &pixels) && IntAddWrapv(
			    pict_target->client->n_pixels, pixels, &pict_target->client->n_pixels))
...
CompareStretch(const DrawParams *params, const DrawParams *other) {
	...
	if (params->flags & StretchSet)
		return (other->crop_width == params->crop_width && other->crop_height == params->crop_height && other->
		        stretch_width == params->stretch_width && other->stretch_height == params->stretch_height);
...
MaybeApplyTransform(PictureBuffer *buffer, DrawParams *params) {
	...
	if (GetScale(params) == GetScale(&buffer->params) && GetSourceX(params) == GetSourceX(&buffer->params) &&
	    GetSourceY(params) == GetSourceY(&buffer->params) && (
		    GetBufferTransform(params) == GetBufferTransform(&buffer->params)) && CompareStretch(
...
Composite(RenderBuffer buffer, RenderTarget target,
	...
	picture_target->buffers_used = XLListPrepend(picture_target->buffers_used, picture_buffer);
}
...
FindSupportedFormats(void) {
	...
	do {
		...
		if (info && !HavePixmapFormat(info->depth,
		                              info->bits_per_pixel))
			continue;
...
MakeCheckWindow(void) {
	...
	flags = (CWColormap | CWBorderPixel | CWEventMask | CWOverrideRedirect);
...
FindSupportedModifiers(int *pair_count_return) {
	...
	for (i = 0; i < ArrayElements(all_formats); ++i) {
		...
		mods = xcb_dri3_get_supported_modifiers_screen_modifiers(reply);
		length = xcb_dri3_get_supported_modifiers_screen_modifiers_length(reply);
		...
		for (j = 0; j < length; ++j) {
			...
			if (mods[j] != DRM_FORMAT_MOD_INVALID && mods[j] != DRM_FORMAT_MOD_LINEAR)
				pair_count += length;
		}
...
InitDrmFormats(void) {
	...
	for (i = 0; i < ArrayElements(all_formats); ++i) {
		...
		for (j = 0; j < all_formats[i].n_supported_modifiers; ++i) {
			...
			drm_formats[n].drm_format = all_formats[i].format_code;
			drm_formats[n].drm_modifier = all_formats[i].supported_modifiers[j];
			n++;
		}
	}
...
BufferFromDmaBuf(DmaBufAttributes *attributes, Bool *error) {
	...
	check_cookie = xcb_dri3_pixmap_from_buffers(compositor.conn, pixmap,
	                                            DefaultRootWindow(compositor.display),
	                                            attributes->n_planes,
	                                            attributes->width,
	                                            attributes->height,
...
BufferFromDmaBufAsync(DmaBufAttributes *attributes,
	...
	record->format = PictFormatForDmabufFormat(attributes->drm_format);
...
IdleEventPredicate(Display *display, XEvent *event, XPointer data) {
	...
	return ((event->type == GenericEvent && event->xgeneric.evtype == PresentIdleNotify) || (
		        event->type == ClientMessage && event->xclient.message_type == _XL_BUFFER_RELEASE));
}
...
WaitForIdle(RenderBuffer buffer, RenderTarget target) {
	...
	while (!IsBufferIdle(buffer, target)) {
	   ...
	   no point in continuing.  */
		if (event.type == GenericEvent && !XGetEventData(compositor.display, &event.xcookie))
			abort();
