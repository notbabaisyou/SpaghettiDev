...
Bool XLHandleXEventForXdgSurfaces(XEvent *event)
{
	XdgRole *role;
	Window window;
...
Commit(Surface *surface, Role *role)
{
	...
	{
		xdg_role->current_state.window_geometry_x = xdg_role->pending_state.window_geometry_x;
		xdg_role->current_state.window_geometry_y = xdg_role->pending_state.window_geometry_y;
		xdg_role->current_state.window_geometry_width = xdg_role->pending_state.window_geometry_width;
		xdg_role->current_state.window_geometry_height = xdg_role->pending_state.window_geometry_height;
	...
	/* This flag means no commit has happened after an
	   ack_configure.  */
	if (!(xdg_role->state & StateWaitingForAckConfigure) && xdg_role->state & StateWaitingForAckCommit)
	{
#ifdef DEBUG_GEOMETRY_CALCULATION
		   ...
		   int max_x, int max_y)
{
	...
	if (role->bounds_width != bounds_width || role->bounds_height != bounds_height)
	{
#ifdef DEBUG_GEOMETRY_CALCULATION
					 ...
					 uint32_t id, struct wl_resource *surface_resource)
{
	...
	if (surface->role || (surface->role_type != AnythingType && surface->role_type != XdgType))
	{
...
void XLXdgRoleAttachImplementation(Role *role, XdgRoleImplementation *impl)
{
	XdgRole *xdg_role;
...
void XLXdgRoleDetachImplementation(Role *role, XdgRoleImplementation *impl)
{
	XdgRole *xdg_role;
...
void XLXdgRoleSendConfigure(Role *role, uint32_t serial)
{
	XdgRole *xdg_role;
...
int XLXdgRoleGetWidth(Role *role)
{
	XdgRole *xdg_role;
	int x, y, x1, y1;
...
int XLXdgRoleGetHeight(Role *role)
{
	XdgRole *xdg_role;
	int x, y, x1, y1;
...
void XLXdgRoleSetBoundsSize(Role *role, int bounds_width, int bounds_height)
{
	XdgRole *xdg_role;
								 ...
								 int *width, int *height)
{
	...
	x1 = (xdg_role->current_state.window_geometry_x + xdg_role->current_state.window_geometry_width - 1);
	y1 = (xdg_role->current_state.window_geometry_y + xdg_role->current_state.window_geometry_height - 1);
...
void XLXdgRoleNoteConfigure(Role *role, XEvent *event)
{
	NoteConfigure(XdgRoleFromRole(role), event);
}

void XLRetainXdgRole(Role *role)
{
	XdgRole *xdg_role;
...
void XLReleaseXdgRole(Role *role)
{
	ReleaseBacking(XdgRoleFromRole(role));
}

void XLXdgRoleCurrentRootPosition(Role *role, int *root_x, int *root_y)
{
	XdgRole *xdg_role;
...
Bool XLXdgRoleInputRegionContains(Role *role, int x, int y)
{
	XdgRole *xdg_role;
...
void XLXdgRoleCancelReconstrainCallback(void *key)
{
	ReconstrainCallback *callback;
...
void XLXdgRoleReconstrain(Role *role, XEvent *event)
{
	XdgRole *xdg_role;
...
void XLXdgRoleMoveBy(Role *role, int west, int north)
{
	int root_x, root_y;
	XdgRole *xdg_role;
...
void XLXdgRoleNoteRejectedConfigure(Role *role)
{
	XdgRole *xdg_role;
...
void XLXdgRoleReplyPing(Role *role)
{
	XdgRole *xdg_role;
