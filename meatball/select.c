...
FindReadTransfer(Atom selection, Atom target, Time time)
{
	...
	{
		if (transfer->selection == selection && transfer->target == target && transfer->time == time)
			return transfer;
...
void SkipChunk(ReadTransfer *transfer)
{
		  ...
		  ptrdiff_t *bytes_after_return)
{
	...
	if (actual_type == None || actual_format != transfer->read_format || rc != Success)
	{
		/* Same goes here.  */
		if (prop_data)
			XFree(prop_data);
...
CanConvertTarget(SelectionOwnerInfo *info, Atom target)
{
	...
	if (target == TARGETS || target == TIMESTAMP)
		return True;
						 ...
						 Window requestor, Atom property)
{
	...
	targets = alloca((3 + info->ntargets) * sizeof *targets);
...
HandlePropertyNotify(XEvent *event)
{
	...
	if (transfer->state & IsIncr && transfer->state & IsWaitingForChunk)
	{
...
Bool HookSelectionEvent(XEvent *event)
{
...
Time GetTransferTime(ReadTransfer *transfer)
{
...
void SetWriteTransferData(WriteTransfer *transfer, void *data)
{
	transfer->data = data;
}

void CompleteDelayedTransfer(ReadTransfer *transfer)
{
...
void StartReading(WriteTransfer *transfer)
{
	TransferBecameReadable(transfer);
}
