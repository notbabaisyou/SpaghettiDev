			   ...
			   int *x, int *y)
{
	...
	dx = (cursor->role.surface->current_state.x * cursor->role.surface->factor);
	dy = (cursor->role.surface->current_state.y * cursor->role.surface->factor);
...
Commit(Surface *surface, Role *role)
{
	...
	if (surface->current_state.frame_callbacks.next != &surface->current_state.frame_callbacks)
		  ...
		  int32_t hotspot_x, int32_t hotspot_y)
{
	...
	if (!surface_resource)
	{
		if (!seen || (wl_resource_get_client(seen->resource) != client))
			return;
	...
	if (!seat->cursor || surface->role != &seat->cursor->role)
	{
		if (!seen || (wl_resource_get_client(seen->resource) != client))
			return;
	}
...
InterceptMotionEventForResize(Seat *seat, XIDeviceEvent *xev)
{
	...
	if (root_x == seat->resize_last_root_x && root_y == seat->resize_last_root_y)
...
NoticeDeviceDisabled(int deviceid)
{
	...
	if (seat && !(seat->flags & IsTestSeat))
	{
	   ...
	   place.  */
		if (seat->flags & IsTextInputSeat && live_seats)
		{
			new = live_seats->data;
...
HandleRawKey(XIRawEvent *event)
{
	...
	if (event->detail < xkb_desc->min_key_code || event->detail > xkb_desc->max_key_code)
		return;
...
HandleRawButton(XIRawEvent *event)
{
	...
	if (seat->resize_in_progress || seat->flags & IsWindowMenuShown)
	{
	...
	complete:
		...
		if (event->detail == seat->last_button && seat->resize_in_progress)
...
SetFocusSurface(Seat *seat, Surface *focus)
{
	...
	seat->focus_destroy_callback = XLSurfaceRunOnFree(focus, ClearFocusSurface, seat);
	...
	if (MaskPopCount(&xev->buttons) == 1)
	{
	   ...
	   progress.  */
		...
		else
		{
			if (seat->drag_last_surface)
			{
				if (!seat->data_source || XLDataSourceCanDrop(seat->data_source))
				{
...
SwapGrabSurface(Seat *seat, Surface *surface)
{
	...
	if (surface)
	{
		...
		seat->grab_surface_callback = XLSurfaceRunAtUnmap(surface, ClearGrabSurface, seat);
	}
}
			   ...
			   double x, double y, Bool preserve_cursor)
{
	...
	if (surface)
	{
		...
		seat->last_seen_surface_callback = XLSurfaceRunOnFree(surface, ClearLastSeenSurface, seat);
...
CanDeliverEvents(Seat *seat, Surface *dispatch)
{
	...
	return (wl_resource_get_client(dispatch->resource) == wl_resource_get_client(seat->grab_surface->resource));
}
...
DispatchEntryExit(Subcompositor *subcompositor, XIEnterEvent *event)
{
	...
	if (event->evtype == XI_Leave || subcompositor != seat->last_seen_subcompositor)
	{
	...
	if (event->mode == XINotifyUngrab && seat->grab_surface)
	...
	if (event->evtype == XI_Enter && event->mode == XINotifyGrab)
...
SetButtonSurface(Seat *seat, Surface *surface)
{
	...
	seat->last_button_press_surface_callback = XLSurfaceRunOnFree(surface, ClearLastButtonPressSurface, seat);
}
...
HandleXkbEvent(XkbEvent *event)
{
	...
	if (event->any.xkb_type == XkbMapNotify || event->any.xkb_type == XkbNewKeyboardNotify)
	{
...
IdentifySeat(WhatEdge *edge, uint32_t serial)
{
	...
	for (tem = live_seats; tem; tem = tem->next)
	{
		...
		if (seat->last_button_serial == serial || seat->last_button_press_serial == serial)
		{
				  ...
				  ResizeEdge edge)
{
	...
	if (!XLWmSupportsHint(_NET_WM_MOVERESIZE) || (seat->flags & IsTestSeat) || getenv("USE_BUILTIN_RESIZE"))
...
Bool XLHandleOneXEventForSeats(XEvent *event)
{
	if (event->type == GenericEvent && event->xgeneric.extension == xi2_opcode)
...
void XLSelectStandardEvents(Window window)
{
	XIEventMask mask;
	size_t length;
						  ...
						  Subcompositor *subcompositor)
{
	...
	else if (event->xgeneric.evtype == XI_Enter || event->xgeneric.evtype == XI_Leave)
	...
	else if (event->xgeneric.evtype == XI_ButtonPress || event->xgeneric.evtype == XI_ButtonRelease)
		DispatchButton(subcompositor, event->xcookie.data);
	else if (event->xgeneric.evtype == XI_KeyPress || event->xgeneric.evtype == XI_KeyRelease)
...
Bool XLMoveToplevel(Seat *seat, Surface *surface, uint32_t serial)
{
...
void XLSeatCancelResizeCallback(void *key)
{
	ResizeDoneCallback *callback;
...
void XLSeatCancelDestroyListener(void *key)
{
	DestroyListener *listener;
...
Bool XLSeatExplicitlyGrabSurface(Seat *seat, Surface *surface, uint32_t serial)
{
	Status state;
	Window window;
	WhatEdge edge;
...
void XLSeatSetDataDevice(Seat *seat, DataDevice *data_device)
{
...
Bool XLSeatIsInert(Seat *seat)
{
	return seat->flags & IsInert;
}

Bool XLSeatIsClientFocused(Seat *seat, struct wl_client *client)
{
	...
	surface_client = wl_resource_get_client(seat->focus_surface->resource);
...
void XLSeatRemoveModifierCallback(void *key)
{
	ModifierChangeCallback *callback;
...
Bool XLSeatResizeInProgress(Seat *seat)
{
	return seat->resize_in_progress;
}

void XLSeatSetTextInputFuncs(TextInputFuncs *funcs)
{
	input_funcs = funcs;
}

int XLSeatGetKeyboardDevice(Seat *seat)
{
	return seat->master_keyboard;
}

int XLSeatGetPointerDevice(Seat *seat)
{
...
void XLSeatDispatchCoreKeyEvent(Seat *seat, Surface *surface, XEvent *event)
{
...
void XLSeatLockPointer(Seat *seat)
{
	seat->flags |= IsPointerLocked;
}

void XLSeatUnlockPointer(Seat *seat)
{
	seat->flags &= ~IsPointerLocked;
}
...
void XLSeatDestroyRelativePointer(RelativePointer *relative_pointer)
{
...
void XLSeatDestroySwipeGesture(SwipeGesture *swipe_gesture)
{
...
void XLSeatDestroyPinchGesture(PinchGesture *pinch_gesture)
{
...
Bool XLSeatApplyExternalGrab(Seat *seat, Surface *surface)
{
	Window window;
	Status state;
	XIEventMask mask;
...
void XLSeatCancelExternalGrab(Seat *seat)
{
...
Bool XLSeatCheckActivationSerial(Seat *seat, uint32_t serial)
{
