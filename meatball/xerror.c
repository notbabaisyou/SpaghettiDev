...
void CatchXErrors(void)
{
	first_error_req = XNextRequest(compositor.display);
	error_caught = False;
}

Bool UncatchXErrors(XErrorEvent *event)
{
...
void ReleaseClientData(ClientErrorData *data)
{
	if (--data->refcount)
		return;
...
SavePendingDisconnectClient(struct wl_client *client)
{
	...
	pending_disconnect_clients = XLListPrepend(pending_disconnect_clients, client);
}
...
void ProcessPendingDisconnectClients(void)
{
	XLList *head;
...
HandleBadAlloc(XErrorEvent *event)
{
	...
	categories = alloca(sizeof *categories * num_clients);
	clients_by_category = alloca(sizeof *clients_by_category * num_clients);
	max_category = MemoryCategoryI;
...
ErrorHandler(Display *display, XErrorEvent *event)
{
	...
	if (first_error_req != -1 && next_request < first_error_req)
		first_error_req = 0;

	if (next_request < next_bad_alloc_serial)
		next_bad_alloc_serial = 0;

	if (first_error_req != -1 && event->serial >= first_error_req)
	{
		error = *event;
		error_caught = True;
