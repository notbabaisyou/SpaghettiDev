...
/* Ensure we have all FOURCC tokens we reference.  Older drm_fourcc.h
 * installations may be missing the X-alpha packed variants. */
#ifndef fourcc_code
 ...
 * Falls back to the first device if the fd path cannot be resolved
 * (correct for single-GPU systems).  Returns EGL_NO_DISPLAY when the
 * extension is absent or no device could be opened.
 */
static EGLDisplay
...
/*
 * On non-Mesa implementations we cannot trust EGL_NATIVE_VISUAL_ID to
 * carry a DRM FOURCC, so we make a best-effort guess from the channel-
 * size attributes alone.
 *
 * We cannot determine channel order from sizes alone, so we assume the
 * most common little-endian BGRA layout (ARGB/XRGB in DRM FOURCC terms).
 * This is wrong on some non-standard implementations but there is no
 * portable EGL way to do better without driver-specific extensions.
 */
static uint32_t
guess_fourcc_from_egl_attribs(const struct basil_egl *egl, EGLDisplay dpy,
                               EGLConfig cfg)
{
    ...
    if (r == 8  && g == 8  && b == 8  && a == 8)  return DRM_FORMAT_ARGB8888;
    if (r == 8  && g == 8  && b == 8  && a == 0)  return DRM_FORMAT_XRGB8888;
    if (r == 10 && g == 10 && b == 10 && a == 2)  return DRM_FORMAT_ARGB2101010;
    if (r == 10 && g == 10 && b == 10 && a == 0)  return DRM_FORMAT_XRGB2101010;
    if (r == 5  && g == 6  && b == 5  && a == 0)  return DRM_FORMAT_RGB565;
    if (r == 5  && g == 5  && b == 5  && a == 1)  return DRM_FORMAT_ARGB1555;
    if (r == 5  && g == 5  && b == 5  && a == 0)  return DRM_FORMAT_XRGB1555;
    if (r == 4  && g == 4  && b == 4  && a == 4)  return DRM_FORMAT_ARGB4444;
    if (r == 4  && g == 4  && b == 4  && a == 0)  return DRM_FORMAT_XRGB4444;

    return 0; /* unrecognised */
}

#define get_config_fourcc(egl, dpy, cfg) \
    guess_fourcc_from_egl_attribs((egl), (dpy), (cfg))
                     ...
                     struct __GLXconfig *chain)
{
    ...
    if (meta.srgb_only) {
        /* Require at least 8 bits for sRGB. */
...
#define TRANSLATE_TO_EQUIVALENT(EGL, GLX) \
    ...
    /* Mesa returns only EGL_WINDOW_BIT support when using the GBM
     * backend; assume support for the other surface types too. */
...
#undef TRANSLATE_TO_EQUIVALENT
    ...
    egl->GetConfigAttrib(disp, egl_config, EGL_DEPTH_SIZE,   &glx_config->depthBits);
    egl->GetConfigAttrib(disp, egl_config, EGL_STENCIL_SIZE, &glx_config->stencilBits);
...
static __GLXconfig *
egl_setup_configs(struct basil_screen *priv)
{
...
basil_screen_probe(ScreenPtr pScreen)
{
    ...
    screen->fbconfigs = egl_setup_configs(priv);
    if (!screen->fbconfigs) {
